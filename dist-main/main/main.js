...
electron_1.app.setName('Notepad by Flo');
const store = new electron_store_1.default();
const isDev = process.env.NODE_ENV === 'development' || !electron_1.app.isPackaged;
const forceClose = new Set();
// Initialize default store settings
if (!store.has('theme'))
    store.set('theme', 'system');
if (!store.has('fontFamily'))
    store.set('fontFamily', 'SF Mono');
if (!store.has('fontSize'))
    store.set('fontSize', 14);
if (!store.has('zoomLevel'))
    store.set('zoomLevel', 100);
if (!store.has('wordWrap'))
    store.set('wordWrap', true);
if (!store.has('statusBarVisible'))
    store.set('statusBarVisible', true);
if (!store.has('spellCheck'))
    store.set('spellCheck', true);
if (!store.has('spellCheckLanguages'))
    store.set('spellCheckLanguages', ['ro', 'en-US']);
function createNewWindow() {
    ...
    const win = new electron_1.BrowserWindow({
        ...
        minWidth: 400,
        minHeight: 300,
        icon: path_1.default.join(appRoot, 'notepad.png'),
        ...
        webPreferences: {
            preload: path_1.default.join(appRoot, 'dist-main', 'preload', 'preload.js'),
    ...
    win.webContents.session.setSpellCheckerLanguages(store.get('spellCheckLanguages'));
    ...
    win.webContents.on('context-menu', (event, params) => {
        ...
        const currentLangs = store.get('spellCheckLanguages');
...
function rebuildMenu() {
    ...
    const template = [
        ...
        },
        {
            label: 'File',
            submenu: [
                { label: 'New Tab', accelerator: 'CmdOrCtrl+T', click: (item, win) => win?.webContents.send('menu-action', 'new-tab') },
                { label: 'New Window', accelerator: 'CmdOrCtrl+Shift+N', click: () => createNewWindow() },
                { label: 'Open...', accelerator: 'CmdOrCtrl+O', click: (item, win) => win?.webContents.send('menu-action', 'open-file') },
                { label: 'Save', accelerator: 'CmdOrCtrl+S', click: (item, win) => win?.webContents.send('menu-action', 'save-file') },
                { label: 'Save As...', accelerator: 'CmdOrCtrl+Shift+S', click: (item, win) => win?.webContents.send('menu-action', 'save-as-file') },
                { label: 'Close Tab', accelerator: 'CmdOrCtrl+W', click: (item, win) => win?.webContents.send('menu-action', 'close-tab') },
                { type: 'separator' },
        ...
        },
        {
            ...
            submenu: [
                ...
                { type: 'separator' },
                { label: 'Find...', accelerator: 'CmdOrCtrl+F', click: (item, win) => win?.webContents.send('menu-action', 'find') },
                { label: 'Replace...', accelerator: 'CmdOrCtrl+H', click: (item, win) => win?.webContents.send('menu-action', 'replace') },
                { label: 'Go To...', accelerator: 'CmdOrCtrl+G', click: (item, win) => win?.webContents.send('menu-action', 'go-to-line') },
                { type: 'separator' },
                {
                    label: 'Spell Check',
                    type: 'checkbox',
                    ...
                    click: (item, win) => {
                        ...
                        electron_1.session.defaultSession.setSpellCheckerEnabled(val);
                        electron_1.BrowserWindow.getAllWindows().forEach(w => {
                            w.webContents.session.setSpellCheckerEnabled(val);
                            w.webContents.send('menu-action', val ? 'spellcheck-on' : 'spellcheck-off');
                        });
                    }
                }
            ]
            ...
            submenu: [
                {
                    ...
                    click: (item, win) => {
                        win?.webContents.send('menu-action', 'toggle-word-wrap');
                    }
                },
                {
                    label: 'Font...',
                    click: (item, win) => win?.webContents.send('menu-action', 'show-font-dialog')
                }
            ]
        },
        {
            label: 'View',
            submenu: [
                { label: 'Zoom In', accelerator: 'CmdOrCtrl+Equal', click: (item, win) => win?.webContents.send('menu-action', 'zoom-in') },
                { label: 'Zoom Out', accelerator: 'CmdOrCtrl+-', click: (item, win) => win?.webContents.send('menu-action', 'zoom-out') },
                { label: 'Reset Zoom', accelerator: 'CmdOrCtrl+0', click: (item, win) => win?.webContents.send('menu-action', 'zoom-reset') },
                { type: 'separator' },
                {
                    label: 'Status Bar',
                    type: 'checkbox',
                    ...
                    click: (item, win) => {
                        win?.webContents.send('menu-action', 'toggle-status-bar');
                    }
                },
                { type: 'separator' },
                {
...
electron_1.ipcMain.handle('file:open', async (event) => {
    ...
    try {
        ...
        electron_1.dialog.showErrorBox('Error reading file', err.message || String(err));
        return null;
    }
});
...
electron_1.ipcMain.handle('file:save', async (_, filePath, content) => {
    try {
        fs_1.default.writeFileSync(filePath, content, 'utf8');
        return true;
    }
    catch (err) {
        electron_1.dialog.showErrorBox('Error saving file', err.message || String(err));
        return false;
    }
});
electron_1.ipcMain.handle('file:saveAs', async (event, content, defaultPath) => {
    const win = electron_1.BrowserWindow.fromWebContents(event.sender);
    if (!win)
        return null;
...
electron_1.ipcMain.handle('store:getStore', () => {
    return store.store;
});
// IPC Window Control Handlers
electron_1.ipcMain.on('window:new', () => {
    createNewWindow();
});
...
electron_1.ipcMain.on('window:setTitle', (event, title) => {
    const win = electron_1.BrowserWindow.fromWebContents(event.sender);
    if (win) {
        win.setTitle(title);
    }
...
electron_1.ipcMain.handle('file:exportPDF', async (event, content, title) => {
    const win = electron_1.BrowserWindow.fromWebContents(event.sender);
    if (!win)
        return false;
    const result = await electron_1.dialog.showSaveDialog(win, {
    ...
    const tempWin = new electron_1.BrowserWindow({
        show: false,
        webPreferences: {
            nodeIntegration: false,
            contextIsolation: true
  ...
  `;
    ...
    try {
        ...
        fs_1.default.writeFileSync(result.filePath, data);
        tempWin.destroy();
        return true;
    }
    catch (err) {
...
electron_1.ipcMain.on('window:toggleMaximize', (event) => {
    const win = electron_1.BrowserWindow.fromWebContents(event.sender);
    if (win) {
        if (win.isMaximized()) {
            win.unmaximize();
